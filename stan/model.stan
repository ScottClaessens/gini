functions {
  ...
  vector ode(real date,      // date
    ...
    real logitU = state[5];     // logit( Pr(urban > 0) )
    real logU   = state[6];     // log(urban)
    real logitG = state[7];     // logit(gini)
    // parameters
    ...
    real bU = exp(theta[5]);     // increase in probability of urban > 0
    real rU = exp(theta[6]);     // rate of urban area production
    real alpha = theta[7];       // continuous-time intercept for gini
    real betaP = theta[8];       // effect of population size on gini
    real betaC = theta[9];       // effect of cropland on gini
    real betaU = theta[10];      // effect of urban area on gini
    real gamma = exp(theta[11]); // damping parameter
    ...
    return to_vector({dlogitP, dlogP, dlogitC, dlogC, dlogitU, dlogU, dlogitG});
  }
}
data {
...
parameters {
  ...
  array[3] vector[11] theta;     // ode parameters over three time periods
  array[29] real<lower=0> tau;   // region SDs
  array[29] vector[N_regions] z; // region-specific effects
  array[5] real<lower=0> lscale; // length-scale for spatial GPs
  array[3] real<lower=0> sigma;  // lognormal variances
...
transformed parameters{
  ...
  betaU_gp     = construct_gp_cholesky(coords, tau[5], lscale[5]) * z[5];
  
  // construct region-specific initial values
  vector[N_regions] init_logit_pop_r  = init_logit_pop  + (tau[6] * z[6]);
  vector[N_regions] init_pop_size_r   = init_pop_size   + (tau[7] * z[7]);
  vector[N_regions] init_logit_crop_r = init_logit_crop + (tau[8] * z[8]);
  vector[N_regions] init_cropland_r   = init_cropland   + (tau[9] * z[9]);
  vector[N_regions] init_logit_urb_r  = init_logit_urb  + (tau[10] * z[10]);
  vector[N_regions] init_urban_r      = init_urban      + (tau[11] * z[11]);
  vector[N_regions] init_gini_r       = init_gini       + init_gini_gp;
  
  // construct region-specific ode parameters
  array[3, N_regions] vector[11] theta_r;
  for (r in 1:N_regions) {
    // period 1
    theta_r[1, r][1]  = theta[1][1]  + (tau[12] * z[12][r]);  // bP
    theta_r[1, r][2]  = theta[1][2]  + (tau[13] * z[13][r]);  // rP
    theta_r[1, r][3]  = theta[1][3]  + (tau[14] * z[14][r]);  // bC
    theta_r[1, r][4]  = theta[1][4]  + (tau[15] * z[15][r]);  // rC
    theta_r[1, r][5]  = theta[1][5]  + (tau[16] * z[16][r]);  // bU
    theta_r[1, r][6]  = theta[1][6]  + (tau[17] * z[17][r]);  // rU
    theta_r[1, r][7]  = theta[1][7]  + alpha_gp[r];           // alpha
    theta_r[1, r][8]  = theta[1][8]  + betaP_gp[r];           // betaP
    theta_r[1, r][9]  = theta[1][9]  + betaC_gp[r];           // betaC
    theta_r[1, r][10] = theta[1][10] + betaU_gp[r];           // betaU
    theta_r[1, r][11] = theta[1][11];                         // gamma
    // period 2
    theta_r[2, r][1]  = theta[2][1]  + (tau[18] * z[18][r]);  // bP
    theta_r[2, r][2]  = theta[2][2]  + (tau[19] * z[19][r]);  // rP
    theta_r[2, r][3]  = theta[2][3]  + (tau[20] * z[20][r]);  // bC
    theta_r[2, r][4]  = theta[2][4]  + (tau[21] * z[21][r]);  // rC
    theta_r[2, r][5]  = theta[2][5]  + (tau[22] * z[22][r]);  // bU
    theta_r[2, r][6]  = theta[2][6]  + (tau[23] * z[23][r]);  // rU
    theta_r[2, r][7]  = theta[1][7]  + alpha_gp[r];           // alpha
    theta_r[2, r][8]  = theta[1][8]  + betaP_gp[r];           // betaP
    theta_r[2, r][9]  = theta[1][9]  + betaC_gp[r];           // betaC
    theta_r[2, r][10] = theta[1][10] + betaU_gp[r];           // betaU
    theta_r[2, r][11] = theta[1][11];                         // gamma
    // period 3
    theta_r[3, r][1]  = theta[3][1]  + (tau[24] * z[24][r]);  // bP
    theta_r[3, r][2]  = theta[3][2]  + (tau[25] * z[25][r]);  // rP
    theta_r[3, r][3]  = theta[3][3]  + (tau[26] * z[26][r]);  // bC
    theta_r[3, r][4]  = theta[3][4]  + (tau[27] * z[27][r]);  // rC
    theta_r[3, r][5]  = theta[3][5]  + (tau[28] * z[28][r]);  // bU
    theta_r[3, r][6]  = theta[3][6]  + (tau[29] * z[29][r]);  // rU
    theta_r[3, r][7]  = theta[1][7]  + alpha_gp[r];           // alpha
    theta_r[3, r][8]  = theta[1][8]  + betaP_gp[r];           // betaP
    theta_r[3, r][9]  = theta[1][9]  + betaC_gp[r];           // betaC
    theta_r[3, r][10] = theta[1][10] + betaU_gp[r];           // betaU
    theta_r[3, r][11] = theta[1][11];                         // gamma
  }
  
  // solve ode
  array[N_regions, N_dates] vector[7] latent;
  for (r in 1:N_regions) {
    // initial values
    ...
    latent[r, 1][5] = init_logit_urb_r[r];
    latent[r, 1][6] = init_urban_r[r];
    latent[r, 1][7] = init_gini_r[r];
...
model {
  ...
  for (p in 1:3) {
    ...
    theta[p][5]  ~ normal(-2, 0.5);  // bU
    theta[p][6]  ~ normal(-2, 0.5);  // rU
    theta[p][7]  ~ normal(0, 0.5);   // alpha
    theta[p][8]  ~ normal(0, 0.5);   // betaP
    theta[p][9]  ~ normal(0, 0.5);   // betaC
    theta[p][10] ~ normal(0, 0.5);   // betaU
    theta[p][11] ~ normal(-2, 0.5);  // gamma
  }
  
  // priors for standardised varying effects, GP, and measurement error
  for (i in 1:29) z[i] ~ normal(0, 1);
  tau ~ exponential(2);
  ...
  for (i in 1:N_obs_urb) {
    real logit_prob = latent[region[urb_idx[i]], date_idx[urb_idx[i]]][5];
    if (urban[i] == 0) {
      target += bernoulli_logit_lpmf(0 | logit_prob);
    } else {
      real mu = latent[region[urb_idx[i]], date_idx[urb_idx[i]]][6];
      target += bernoulli_logit_lpmf(1 | logit_prob);
      target += lognormal_lpdf(urban[i] | mu, sigma[3]);
    }
  }
  
  // likelihood for gini
  for (i in 1:N_obs_gini) {
    real mu = latent[region[gini_idx[i]], date_idx[gini_idx[i]]][7];
...
generated quantities {
  ...
  array[N_regions, 121] vector[7] regional_latent_rep;
  ...
  for (i in 1:N_obs_urb) {
    ...
    if (bernoulli_logit_rng(logit_prob) == 0) {
      ...
      real mu = latent[region[urb_idx[i]], date_idx[urb_idx[i]]][6];
      urban_rep[i] = lognormal_rng(mu, sigma[3]);
    }
  }
  ...
  for (r in 1:N_regions) {
    ...
    regional_latent_rep[r, 1][5] = init_logit_urb_r[r];
    regional_latent_rep[r, 1][6] = init_urban_r[r];
    regional_latent_rep[r, 1][7] = init_gini_r[r];
